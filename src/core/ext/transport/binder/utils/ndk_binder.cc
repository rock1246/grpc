...
void* GetNdkBinderHandle() {
  ...
  gpr_log(GPR_INFO, "Rokya: GetNdkBinderHandle() has GPR_USE_NDK_BINDER");
  static void* handle = dlopen("libbinder_ndk.so", RTLD_LAZY);
  return handle;
#else
  gpr_log(GPR_INFO, "Rokya: GetNdkBinderHandle() has no GPR_USE_NDK_BINDER");
  return nullptr;
#endif
}
...
#define FORWARD(name)                                           \
  ...
  static func_type ptr = []() {                                 \
    ...
    if (handle != nullptr) {                                    \
      gpr_log(GPR_INFO, "Rokya: 1110 call handle " #name); \
      return reinterpret_cast<func_type>(dlsym(handle, #name)); \
    } else {                                                    \
      gpr_log(GPR_INFO, "Rokya: 1110 call JNI_" #name); \
      return JNI_##name;                                        \
    };                                                          \
  }();                                                          \
  return ptr
